...
After compounding, check if it's time for an architecture review. Look for the "Last architect review" date in `specs/ARCHITECTURE.md` (set by `/architect`). If it doesn't exist, `/architect` has never been run. If it does exist, count `feat(` and `fix(` commits since that date using `git log --oneline --since="[date]" | grep -cE "^[a-f0-9]+ (feat|fix)"`.

If 3 or more features/patches have shipped since the last architect review (or if one has never been run):

```
Architecture check: [N] features/patches since last /architect run.
Architectural drift accumulates silently. Consider running /architect to check for god modules, layer violations, and structural rot before the next feature.
```
