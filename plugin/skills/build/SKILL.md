...
The `[human-writes]` tag in the commit message marks it in git history so `/upfront:retro` and future analysis can distinguish human-written phases from AI-written ones.
...
### Post-edit hook (auto-lint + auto-test)

After pre-flight, generate a hook that runs the project's lint and test commands **after every file edit**. This catches mistakes at the edit level — the agent can't accumulate broken files across a phase.

#### Step 1: Discover existing commands

Read the project's config files to find the actual test and lint commands:

| Look for | Test command | Lint command |
|----------|-------------|-------------|
| `package.json` scripts | `npm test` / `npm run test` | `npm run lint` |
| `Makefile` / `Justfile` | `make test` | `make lint` |
| `go.mod` | `go test ./...` | `go vet ./...` (+ `golangci-lint run` if installed) |
| `Cargo.toml` | `cargo test` | `cargo clippy` |
| `pyproject.toml` | `pytest` / `python -m pytest` | `ruff check` (or what's configured) |
| `build.gradle` / `gradlew` | `./gradlew test` | `./gradlew check` |
| `pom.xml` | `mvn test` | `mvn checkstyle:check` (if configured) |
| `*.csproj` / `*.sln` | `dotnet test` | `dotnet format --verify-no-changes` |

Use what the project already has. Don't guess — read the config.

#### Step 2: Force the decision if missing

**If no test command exists:** Do not proceed without one. Present a strong recommendation:

```
This project has no test runner configured. The post-edit hook needs one.

Recommended for [ecosystem]:
  [ecosystem-specific recommendation]

Want me to set this up now? This is Phase 0 work — it gets committed before any feature code.
```

**Recommendations by ecosystem:**
- TypeScript/JS: vitest (fastest, ESM-native, compatible with Jest API)
- Python: pytest (standard, rich plugin ecosystem)
- Go: built-in (`go test`) — if this is missing, the project has deeper problems
- Rust: built-in (`cargo test`) — same
- JVM/Gradle: JUnit 5 via `./gradlew test`
- JVM/Maven: JUnit 5 via `mvn test`
- .NET: xUnit via `dotnet test`

If the user declines a test runner, note it in the progress file and skip the post-edit hook. The phase-level verification still runs, but the tighter edit-level feedback loop is lost.

**If no lint command exists:** Same pattern — recommend the standard for the ecosystem (ruff for Python, eslint for TS, clippy for Rust, etc.) and push for setup.

#### Step 3: Generate the hook

Once you have the test and lint commands, create `.claude/hooks/post-edit.sh` in the working directory:
...
# --- Generated by /upfront:build pre-flight ---
LINT_CMD="[DETECTED_LINT_COMMAND]"
TEST_CMD="[DETECTED_TEST_COMMAND]"

# Run lint on the changed file (or full project if tool doesn't support single-file)
record "lint" bash -c "$LINT_CMD"

# Run tests scoped to the changed file's area if possible
record "test" bash -c "$TEST_CMD"
...
Replace `[DETECTED_LINT_COMMAND]` and `[DETECTED_TEST_COMMAND]` with the actual commands from Step 1.

If the project's test command supports targeting a specific file or directory (most do), scope the test to the changed file's area rather than running the full suite on every edit. For example:
- Go: `go test -race -count=1 ./$(dirname "$FILE")/...`
- pytest: `python -m pytest $(dirname "$FILE") -x -q`
- vitest/jest: `npx vitest run "$FILE"` (if it's a test file)
- cargo: `cargo test` (no scoping, but fast enough)

#### Step 4: Register the hook

Add to the project's `.claude/settings.local.json` (not committed):
...
Tell the user: "Post-edit hook installed — lint and tests run after every file edit. Mistakes get caught immediately."

#### Cleanup

Remove the hook and settings entry after the build completes (in the merge-back step). The hook is a build-time accelerator — too noisy for normal development where you might be mid-thought across multiple files.

If the user wants to keep it permanently, they can move it from `.claude/settings.local.json` to `.claude/settings.json`.
...
To resume: run /upfront:build — it will detect the existing worktree
...
**What the agent got wrong:** [patterns of AI mistakes — useful for future /upfront:plan and /build]
...
After compounding, check if it's time for an architecture review. Look for the "Last architect review" date in `specs/ARCHITECTURE.md` (set by `/upfront:architect`). If it doesn't exist, `/upfront:architect` has never been run. If it does exist, count `feat(` and `fix(` commits since that date using `git log --oneline --since="[date]" | grep -cE "^[a-f0-9]+ (feat|fix)"`.

If 3 or more features/patches have shipped since the last architect review (or if one has never been run):

```
Architecture check: [N] features/patches since last /upfront:architect run.
Architectural drift accumulates silently. Consider running /upfront:architect to check for god modules, layer violations, and structural rot before the next feature.
```
...
6. **Close spike debt:** If the spec references an originating spike (look for "Originated from spike" in the spec), find the matching spike entry in `specs/DEBT.md` and mark each debt item as resolved:
   - Change `- [ ]` to `- [x]` for items addressed by the build (spec, tests, architecture review)
   - Update the spike's **Status** to `resolved — replaced by [feature-name]`
   - Leave any items that weren't addressed (e.g., if the build didn't cover a specific hardcoded config, it stays open)
...
If the user re-runs `/upfront:build` on a plan with some phases already completed, follow this crash recovery protocol before continuing.
