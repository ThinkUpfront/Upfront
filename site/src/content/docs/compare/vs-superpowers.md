...
| **Rapid prototyping** | `/upfront:spike` — build a throwaway prototype, test the idea, track debt | Not addressed |
...
| **Tech debt tracking** | `/upfront:spike` logs debt automatically, `/upfront:build` logs deferred items, surfaced in retros and health checks | None |
| **Tooling health** | `/upfront:upgrade` checks plugin version, guardrails, linter/formatter, pre-commit hooks, instruction file quality, debt balance | None |
